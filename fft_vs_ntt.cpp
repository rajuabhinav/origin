...
void fft(vector<complex<double> > &a, bool inv, complex<double> wn) {
	if (a.size() < 2) {
		return;
	}
	int n = a.size();
	complex<double> one(1.0, 0), w(1.0, 0);
...
void multiply(vector<int> &a, vector<int> &b) {
	...
	fft(ac, false, complex<double>(cos(ang), sin(ang)));
	fft(bc, false, complex<double>(cos(ang), sin(ang)));
	for (int i = 0; i < ac.size(); i++) {
		ac[i] *= bc[i];
	}
	ang = -ang;
	fft(ac, true, complex<double>(cos(ang), sin(ang)));
	for (int i = 0; i < ac.size(); i++)
		a[i] = ac[i].real() + 0.5;
}
